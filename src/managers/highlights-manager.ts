...
export async function exportHighlights(): Promise<void> {
	try {
		...
		const fileName = `hexel-capture-highlights-${timestamp}.json`;
			...
			if (navigator.share) {
				try {
					await navigator.share({
						...
						title: 'Exported Hexel Capture Highlights',
						text: 'Here are your exported highlights from Hexel Capture.'
					});
				} catch (error) {
